...
| 文件名 | 类型 | 说明 |
| --- | --- | --- |
| `失败案例1_当前环境依赖缺失无法启动.png` | 环境失败 | 当前机器缺少 Gradio、LitServe、Transformers、SAM2 等依赖，因此无法启动服务生成新的目标识别失败截图 |

说明：由于当前环境依赖不完整，本次未能现场生成“目标太小、背景复杂、文本识别失败”这一类真实推理失败截图。正式演示前补齐依赖后，可以用小目标、遮挡目标或复杂背景图片测试并补充。
